...
void DenseLayer::InputGradientCode(const symbolic::Expression &input_index,
                                   codegen::Generator *cg) const {
  symbolic::Expression gradient_components = 0.0;
  for (size_t node = 0; node < dimensions_.num_outputs; ++node) {
    symbolic::Expression output = OutputSymbol(node);
    gradient_components += generator_.GRADIENT(node) * output.Derive(generator_.I(input_index).to_string());
  }
  ...
  //}
  cg->AppendLineOfCode("return " + gradient_components.to_string() + cg->linesep());
}
...
void DenseLayer::WeightGradientCode(
  ...
  symbolic::Expression output = OutputSymbol(node);
  symbolic::Expression retval = generator_.GRADIENT(node) * output.Derive(generator_.W(node, edge).to_string());
