...
class KalmanFilter {
  public:
    ...
    using ProcessNoiseMatrix = Matrix<kNumStates, kNumStates, symbolic::Expression>;
    ...
    KalmanFilter(StateMatrix state_matrix, ControlMatrix
      control_matrix, ProcessNoiseMatrix process_noise, SensorTransform sensor_transform) :
      state_transition_(state_matrix), control_matrix_(control_matrix),
      process_noise_(process_noise),
      sensor_transform_(sensor_transform) {}

    // Initialize sets initial values for X and P.
    void initialize(Time time_s, StateVector initial_state, StateCovariance
    state_covariance) {
    ...
    void ReportControl(Time time_s, ControlVector controls) {
      auto expression_evaluator = [time_s, this](symbolic::Expression exp) -> Number {
      ...
      Matrix<kNumStates, kNumStates, Number> process_noise = process_noise_.Map(expression_evaluator);
    ...
    std::tuple<StateVector, StateCovariance> PredictState(Time time_s) const {
      ...
      StateVector estimation = state_transition * state_ + control_matrix * last_control_;
    ...
    void ReportSensorReading(Time time_s, SensorVector sensors, SensorCovariance
    sensor_covariance) {
